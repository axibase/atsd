...
Set status to `Enabled` and enter one or multiple email addresses, separated by comma, semi-colon or space character.
...
| Recipients | One or multiple email subscribers to receive messages. Use a comma, semi-colon or space to separate multiple addresses. |
