...
Notifications can be triggered by numerous events such as when a window changes status or maintains the `REPEAT` status as the expression continues to evaluate to `true` for a defined interval.

## Email Client

Messages generated by rule engine are sent via built-in [mail client](../administration/mail-client.md) which is configurable on the **Settings > Mail Client**.
...
| Recipients | One or more email subscribers which receive messages.<br>Use a comma, semi-colon, or whitespace to separate multiple addresses. |
...
Complete message **Subject** and **Text**.
