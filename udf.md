...
Метод `interpolte` объектов `Series` и `SeriesCollection` выполняет интерполяцию ряда (каждого ряда в коллекции). Описание параметров метода смотрите в документации этих классов.
