...
Особенность расчета заключается в том, что несмотря на трансляцию `waprice` для каждого режима по отдельности, значение средневзвешенной цены основывается на сделках из **всех режимов торгов**, включая сделки в режиме РПС с ЦК где могут регистрироваться сделки на сотни миллионов рублей с отклонением от рыночной цены до 5%. Таким образом, значение `waprice` как правило не равно результату деления `valtoday/voltoday`.

Приводимый ниже анализ преследует задачу обосновать рекомендацию не использовать `waprice` в качестве ценового показателя основного режима (TQBR для акций). В качестве замены предлагается самостоятельно вычислять значение `valtoday/voltoday`.
...
Последствия данной особенности в расчете заключаются в том, что показатель `waprice`, а также связанные с ним `prevwaprice` и `priceminusprevwa`, нельзя использовать в качестве ценовой оценки хода торгов в основном режиме.
