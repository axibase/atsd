...
Услуги DMA предоставляются как правило крупными брокерами. На момент углубленного изучения данного вопроса и выбора поставщика услуг для размещения АТСД в целях оптимизации в боевых условиях в 2019 г, мы рассматривали [Финам](https://broker.finam.ru/dma/) и [БКС](https://broker.ru/services/dma/stock-fastfix). Выбор был сделан в пользу Финама и присутствующие в данной статье ценовые и технические характеристики отражают установку у данного брокера.

## Схема интеграции

Схема доставки данных выглядит следуюшим образом:

![](./images/fast-model-3.png)

Шлюз FAST отправляет данные на сервер клиента в так называемой зоне [колокации](https://www.moex.com/s154#cs-tab3). Это выделенный сегмент сети, физически приближенный к зоне, в которое расположено аппаратное обеспечение торговых систем самой биржи. В зоне колокации допустимо размещение вычислительных ресурсов клиентов, и при этом сетевая задержка в данном сегменте сети минимальна.
...
Дополниительно необходимо произвести увеличение сетевых буферов, и настроить синхронизацию системного времени с сервисами точного времени биржи.
...
Внешная сеть является точкой входа для управления сервером с доступом по SSH протоколу. Этот же интерфейс используется для отправки данных в зону колокации, либо из зоны колокации в виде пока данных от FAST консьюмеров и резервного копирования логов и снепшотов в неторговое время.
...
Подписка для срочного рынка предлагается в двух версиях: базовый и расширенный. На базовом уровне данные приходят с задержкой. Это происходит в связи с применением так называмой "нарезки", когда данные отправляются не поточно, а пачками. Расширенный уровень позволяет получать данные без задержки  в формате "полный лог заявок", однако требует в 5 раз больше пропуской способности сети и оформления дополнительной подписки. 
...
Ежемесячная плата за каждый рынок составляет 2000 рублей, и дополнительно 14000 рублей в месяц для полного лога заявок Срочного рынка. Также уплачивается единоразовая стоимость подключения к каждому рынку в размере 2000 рублей. 

![](./images/fast-prices.png)

Дополнительно необходимо оплатить 7500 рублей за каждый рынок в месяц за Non-display использование, однако при подключении к FAST как участника торгов через брокера, имеющего соответствующую лицензию и договор с биржей, данная плата не взымается.
...
По результатам нашего [мониторинга](../latency_monitoring.md), медиана задержки публикации для потока заявок составляет 275 микросекунд, 95% персентиль - 800 микросекунд. Для потока сделок эти показатели составляют 350 и 1500 микросекунд соответственно.

![](./images/fast-trades-latency.png)

При этом существует значительная разница в задержке по типам сообщений (`MDUpdateAction`, `MDEntryType`) и потокам. Например, отмена заявки происходит существенно быстрее, чем ее обновление или исполнение.

Также нами наблюдается задержка особого типа, связанная с количеством транзакций в полученных сообщениях. Речь идет о крупных рыночных или исполняемых лимитных заявках, которые порождают большое количество транзакций. В связи с последовательной отправкой сообщений, разбитых на несколько UDP фрагментов, состояние книги заявок не может быть рассчитано до момента применения последней транзакции в последнем фрагменте.

### Сетевая задержка

При использовании виртуального сервера медиана задержки между временем получения сообщения и `SendingTime` составляет 300 микросекунд, а 95% персентиль - 2000 микросекунд. Задержка не зависит от типа сообщения и канала. Данная задержка зависит от вычислительных и сетевых ресурсов провайдера DMA услуг.
...
Проблемы со связью у провайдера Relline в виде недоступности систем нами зафиксированы не были, однако скачки задержек возникают 2-3 раза в неделю и продолжаются 5-10 минут.

Для получения оперативной информации от службы поддержки DMA провайдера мы пользуемся Telegram-каналом, в котором брокер публикуют сообщения о сбоях в системах по факту их обнаружения:

![](./images/dma-finam-tg.png)
...
Оплата DMA услуг осуществляется путем списания ежемесячной абонентской платы со счета единой денежной позиции. Это позволяет учесть расходы на данные услуги при расчете налогооблагаемой базы от операций с ценными бумагами.