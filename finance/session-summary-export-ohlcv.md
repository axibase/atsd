...
| `session` | Session name.  All session types are retrieved if not specified. |
| `stage` | Session stage. All stages are retrieved if not specified. |
| `fields` | List of statistics to include in the response.<br>If specified, `datetime`, `symbol`, and at least one [statistics](statistics-fields.md) field must be present. |
