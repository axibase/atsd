...
| `exactMatch` | boolean | `tags` match operator. _Exact_ match if `true`, _partial_ match if `false`.<br>Default: `false` (_partial_ match).<br>_Exact_ match selects series with exactly the same `tags` as requested.<br>_Partial_ match selects series with tags that contain requested tags but can also include additional tags.|
