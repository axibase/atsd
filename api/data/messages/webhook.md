...
A set of reserved parameter names, specified in the query string, are provided to control mappings and discard extraneous data.
...
Primitive JSON fields (string, number, boolean) are converted into message **tags**, where tag name equals the field's full name (contains parent object names, separated by `.` character) and tag value equals field value.

`string` fields are automatically unquoted by removing leading and trailing single or double quotes, if present.
