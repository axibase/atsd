...
| **Name** | **Description** |
|---|---|
| `json.parse` | Converts a string representation of JSON into a map consisting of keys and values.<br>The field's value is added to the map with a key set to its full name, created by appending the field's local name to the full name of its parent object using `.` as a separator.<br>If the field is an array element, its local name is set to element index `[i]` (index `i` starts with `0`).|
