...
|disk_totalspace|Size, in bytes, of the ATSD store.|	
...
|email_notifications_per_minute |Average number of email notifications sent by ATSD Rule Engine per minute. |	
|expired_metric_received_per_second |Number of metrics that satisfy the following rule: `now - timestamp > 1 hour`. |	
|filtered_metric_received_per_second|Average number of metrics which have been filtered due to triggering of `Invalid Value Action`.|	
|forward_metric_received_per_second |Number of metrics that satisfy the following rule: `timestamp - now > 1 hour`. |	
|gc_invocations_per_minute_MarkSweepCompact |Average number of MarkSweepCompact gc invocations per minute.|	
|gc_time_percent_MarkSweepCompact |Percentage of time spent by MarkSweepCompact.|	
|hbase.thread_pool_active|Size (number of active clients) of the `htable.executor.PoolSize`.|	
|hbase_scans_per_second |Average number of HBase searches per second. |
...
|invalid_message_received_per_second | Average number of invalid messages received per second. |
|invalid_property_received_per_second| Average number of invalid properties received per second.|	
...
|message_reads_per_second|Average number of messages read from disk per second.|	
|message_received_per_second | Average number of messages received per second. |	
|message_writes_per_second | Average number of messages stored on disk per second. |	
|metric_append_concat_per_second|Average number of series commands with flag `append` immediately before data record to HBase.|	
|metric_append_per_second|Average number of series commands with flag `append` which have been checked for metrics, entities `enable` flag and persistence; stored before processing by Rule Engine.|	
|metric_reads_per_second |Average number of metrics read from disk per second. |
|metric_received_per_second |Average number of metrics received per second. |	
|metric_writes_per_second |Average number of metrics stored on disk per second. |
|network_command_ignored_per_second|Average number of TCP and UDP commands were skipped.|	
|network_command_malformed_per_second| Average number of TCP and UDP commands were dropped because they were not valid.|	
|non_persistent_metric_received_per_second|Average number of not persistent metric received per second.|	
...
|property_deleted_per_second|Average number of properties deleted from disk per second.|	
|property_reads_per_second |Average number of properties read from disk per second. |
|property_received_per_second |Average number of properties received per second.|	
|property_writes_per_second |Average number of properties stored on disk per second. |	
|series_pool_active_count|Number of active threads which are add series to ATSD.|	
...
|web_service_notifications_per_minute | Average number of web service notifications sent by ATSD Rule Engine per minute.|
