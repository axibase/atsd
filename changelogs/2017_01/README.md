...
[`JOIN USING ENTITY`](../../sql/README.md#join) didn't merge rows as expected if tag names had the same names but different values. For example, the same tag `tag1` with different values `a` and `b`.
